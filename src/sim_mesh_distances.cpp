...
#include "sim_mesh_distances/sim_mesh_distances.hpp"
#include <thread>
#include <unistd.h>
...
int SimMeshDistPrivate::udp_open()
{
...
void SimMeshDistPrivate::calculate_distances()
{
...
void SimMeshDistPrivate::handle_msg()
{
    _buffer[_buf_size] = 0;
...
SimMeshDist::SimMeshDist()
: Node("SimMeshDist"),
  _impl(std::make_unique<SimMeshDistPrivate>())
{
...
void SimMeshDist::get_address(std::string &addr, uint16_t &port)
{
...
void udp_worker(std::shared_ptr<SimMeshDist> node)
{
    while (rclcpp::ok())
    {
        node->udp_poll();
...
int main(int argc, char * argv[])
{
    ...
    std::shared_ptr<SimMeshDist> node = std::make_shared<SimMeshDist>();
